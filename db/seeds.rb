...
Refinery::I18n.frontend_locales.each do |lang|
  ...
  Refinery::User.find_each do |user|
    user.plugins.where(name: 'refinerycms-order').first_or_create!(
      position: (user.plugins.maximum(:position) || -1) +1
    )
  end if defined?(Refinery::User)

  Refinery::Page.where(link_url: (url = "/order/orders")).first_or_create!(
    title: 'Orders',
    deletable: false,
    menu_match: "^#{url}(\/|\/.+?|)$"
  ) do |page|
...
# Refinery::Products::Product.first_or_create( name: 'Seal Pops', price: 1000)
# Refinery::Images.first_or_create( name: 'Seal Pops', price: 1000)
